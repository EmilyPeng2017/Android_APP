...
public class Converter {
	...
	/*To get decimal string from byte array. 
	  Caution: the data we get from byte array is in hex,so we don't need 
	           a function to convert decimal string to byte array.
	           Only hex string can be converted to byte array. 
	*/
	public static String getDecString(byte[] raw, int len) {
		byte[] dec = new byte[3 * len];
		int index = 0;
		int pos = 0;
		...
		for (byte b : raw) {
			...
			int v = b & 0xFF;
			int bai=v/100;
			int shi=(v-bai*100)/10;
			int ge=v-bai*100-shi*10;
			dec[index++] = HEX_CHAR_TABLE[bai];
			dec[index++] = HEX_CHAR_TABLE[shi];
			dec[index++] = HEX_CHAR_TABLE[ge];
		}
